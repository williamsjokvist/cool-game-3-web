...
const NavItem: FunctionComponent<NavItemProps> = (props) => {
  ...
  return (
    <li className="group nav-link-container relative flex justify-between items-center h-32 w-32 bg-[#1C1C25] rounded-xl">
      ...
      {props.toolTip && (
        <span className="z-50 whitespace-nowrap absolute left-[155px] bg-black px-5 py-2 rounded-xl group-hover:block hidden text-xs text-center">
          {props.toolTip}
        </span>
      )}
    </li>
...
const Nav: FunctionComponent = (props) => {
  return (
    <nav className="fixed left-[-2px] top-0 h-screen font-arial text-white text-md mb-12 flex items-center">
      <ul className="rounded-r-xl transition-all flex flex-col gap-4 border-[#25252F] bg-[#25252F] hover:bg-[#2D2D38] hover:border-[#97979B] border-[0.5px] border-solid p-3 shadow-sm">
